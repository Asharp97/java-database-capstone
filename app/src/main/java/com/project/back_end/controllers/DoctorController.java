...
public class DoctorController {
  ...
  public ResponseEntity<List<String>> getDoctorAvailability(String user, long doctorId, LocalDate date, String token) {
    if (!service.validateToken(token, user))
      return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();

    // The service method handles returning OK or whatever status is appropriate
  ...
  @PostMapping("/{token}")
  public ResponseEntity<HttpStatus> saveDoctor(Doctor doctor, @PathVariable String token) {
    if (!service.validateToken(token, "admin"))
      return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();

    if (doctorService.saveDoctor(doctor) == 0)
  ...
  @PutMapping("/{token}")
  public ResponseEntity<HttpStatus> updateDoctor(Doctor doctor, @PathVariable String token) {
    if (!service.validateToken(token, "admin"))
      return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();
  ...
  public ResponseEntity<HttpStatus> deleteDoctor(@PathVariable long id, @PathVariable String token) {
    if (!service.validateToken(token, "admin"))
      return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();
