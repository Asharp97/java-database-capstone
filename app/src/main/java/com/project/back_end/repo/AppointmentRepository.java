...
@Query("SELECT a FROM Appointment a LEFT JOIN a.doctor d WHERE d.id = :doctorId AND a.appointmentTime BETWEEN :start AND :end")
...
@Query("SELECT a FROM Appointment a LEFT JOIN a.doctor d LEFT JOIN a.patient p WHERE d.id = :doctorId AND p.name LIKE %:patientName% AND a.appointmentTime BETWEEN :start AND :end")
