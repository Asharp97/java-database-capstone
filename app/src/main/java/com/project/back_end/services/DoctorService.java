...
public class DoctorService {
  ...
  public List<String> getDoctorAvailability(Long doctorId, LocalDate date) {
    ...
    return appointments.stream().map(a -> a.getAppointmentTime().toString()).toList();
  }
  ...
  public List<Doctor> filterDoctorsByTime(List<Doctor> doctors, String timePeriod) {
    return doctors.stream().filter(doctor -> doctor.getAvailableTimes().stream().anyMatch(timeStr -> {
